...
# Set up parameters using argparse
...
# Fill our list of image np-arrays using a for-loop
def visualiser_rapidement_img(directoryPath, count):
    ...
    for signal in image_dataset:
        ...
        # Calculate average grayscale value for each image
        img_grayscale_value = np.mean(img_float)
        # Add it to the list
        data.append(img_grayscale_value)

    np_data_grayscale_value = np.array(data, dtype='float32')
    max_light = np.argmax(np_data_grayscale_value)
    ...
    # Save the image with the highest brightness
